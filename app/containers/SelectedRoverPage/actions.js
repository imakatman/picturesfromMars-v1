  ...
  return function (dispatch, getState) {
    if(getState().getDataByRover[rover][camera]['abortLoop']){
      ...
      return fetch(`https://api.nasa.gov/mars-photos/api/v1/rovers/` + rover + `/photos?sol=` + sol + `&camera=` + camera + `&page=` + page + `&api_key=` + apiKey).then(response => {
        let parameters = [rover, sol, page, camera, cameraFullName, cameraIndex];
        return dispatch(handleErrors(response, fetchRoverImages, ...parameters));
        ...
        console.log("catch fetchroverimages " + err);
        let parameters = [rover, sol, page, camera, cameraFullName, cameraIndex];
        // return dispatch(handleErrors(err, fetchRoverImages, ...parameters));
      });
    }
  };
}
